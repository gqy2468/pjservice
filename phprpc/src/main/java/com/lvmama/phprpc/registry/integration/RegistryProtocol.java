...
import com.lvmama.phprpc.registry.RegistryService;
...
public class RegistryProtocol implements Protocol {
    ...
    private <T> void doChangeLocalExport(final Invoker<T> originInvoker, URL newInvokerUrl){
        ...
        if (exporter == null){
            logger.warn(key, new IllegalStateException("error state, exporter should not be null"));
    ...
    private <T> Invoker<T> doRefer(Cluster cluster, Registry registry, Class<T> type, URL url) {
        ...
        URL subscribeUrl = new URL(Constants.CONSUMER_PROTOCOL, NetUtils.getLocalHost(), 0, type.getName().replace("$Iface",""), directory.getUrl().getParameters());
        ...
        return cluster.join((Directory<T>) directory);
    }
