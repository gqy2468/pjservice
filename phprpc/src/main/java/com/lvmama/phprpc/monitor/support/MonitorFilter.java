 ...
 */
package com.lvmama.phprpc.monitor.support;
...
import com.lvmama.phprpc.common.extension.Activate;
...
import com.lvmama.phprpc.rpc.Filter;
...
public class MonitorFilter implements Filter {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        if (invoker.getUrl().hasParameter(Constants.MONITOR_KEY) && !"com.lvmama.phprpc.monitor.MonitorService".equals(invoker.getUrl().getServiceKey())) {
    ...
    private void collect(Invoker<?> invoker, Invocation invocation, Result result, RpcContext context, long start, boolean error) {
        try {
            ...
            monitor.collect(new URL(Constants.COUNT_PROTOCOL,
                                ...
                                Constants.OUTPUT_KEY, output).toString());
        } catch (Throwable t) {
